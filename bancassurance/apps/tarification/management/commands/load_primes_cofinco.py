...
class Command(BaseCommand):
    help = 'Charge les grilles tarifaires COFINCO (Likama/Mobateli + Elikia + Emprunteur)'

    def handle(self, *args, **options):
        self.stdout.write("🚀 Chargement des grilles COFINCO...")
        
        try:
            banque_boa = Banque.objects.get(code_banque='COFINCO')
        except Banque.DoesNotExist:
            self.stdout.write(self.style.ERROR("❌ Banque COFINCO non trouvée !"))
            self.stdout.write("Créez d'abord la banque COFINCO avec le code 'COFINCO'")
            return
        
        # === LIKAMA COFINCO (= MOBATELI) ===
        inserted_likama, updated_likama = self.load_likama_mobateli(banque_boa)
        
        # === ELIKIA SCOLAIRE COFINCO ===
        ...
        self.stdout.write(self.style.SUCCESS(f"   • Likama/Mobateli COFINCO : {inserted_likama} insérés, {updated_likama} mis à jour"))
        self.stdout.write(self.style.SUCCESS(f"   • Elikia COFINCO : {inserted_elikia} insérés, {updated_elikia} mis à jour"))

    def load_likama_mobateli(self, banque):
        """
        Charge les primes Likama COFINCO (= Mobateli COFINCO)
        Utilise la table TablePrimesMobateli avec banque=COFINCO
        """
        self.stdout.write("\n📊 Chargement LIKAMA/MOBATELI COFINCO (DTC/IAD)...")
        
        # Grille Likama COFINCO (identique à Mobateli BCI d'après les images)
        grille = [
    ...
    def load_elikia(self, banque):
        """
        Charge les primes Elikia Scolaire COFINCO
        Utilise la table TablePrimesElikia avec banque=COFINCO
        """
        self.stdout.write("\n📊 Chargement ELIKIA SCOLAIRE COFINCO...")
        
        # Grille Elikia COFINCO (identique à BCI d'après les images)
        grille = [
            # Rente 200 000

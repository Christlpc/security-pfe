# apps/tarification/management/commands/load_primes_bci.py
...
class Command(BaseCommand):
    help = 'Charge les grilles tarifaires BCI (Elikia + Mobateli)'

    def handle(self, *args, **options):
        self.stdout.write("🚀 Chargement des grilles BCI...")
        
        try:
            banque_bci = Banque.objects.get(code_banque='BCI')
        except Banque.DoesNotExist:
            self.stdout.write(self.style.ERROR("❌ Banque BCI non trouvée !"))
            return
        
        date_debut = timezone.now().date()
        
        # === ELIKIA SCOLAIRE ===
        inserted_elikia, updated_elikia = self.load_elikia(banque_bci, date_debut)
        
        # === MOBATELI ===
        inserted_mobateli, updated_mobateli = self.load_mobateli(banque_bci, date_debut)
    ...
    def load_elikia(self, banque, date_debut):
        """Charge les primes Elikia Scolaire (d'après bci_elikia.png)"""
        self.stdout.write("\n📊 Chargement ELIKIA SCOLAIRE...")
        
        # Grille Elikia (bci_elikia.png)
        grille_elikia = [
            # Rente 200 000
    ...
    def load_mobateli(self, banque, date_debut):
        """Charge les primes Mobateli (d'après bci_mobateli.png)"""
        self.stdout.write("\n📊 Chargement MOBATELI...")
        
        # Grille Mobateli (bci_mobateli.png)
        grille_mobateli = [
            # Capital 2 000 000
...
├── TablePrimesElikia         → Elikia Scolaire (BCI) - PRIMES FORFAITAIRES
├── TablePrimesMobateli       → Mobateli (BCI) - PRIMES FORFAITAIRES
